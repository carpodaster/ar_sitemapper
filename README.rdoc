...
<tt>RAILS_ROOT/config/sitemaps.yml</tt>.
...
==== Sample YAML file
  ...
  local_path: <%= File.join RAILS_ROOT, "public", "sitemaps" %>
  ping: true
  index:
...
=== Rake task
This is a simple rake task that loads and builds sitemaps from config/sitemaps.yml

  namespace :sitemapper do
    desc "Rebuilds all sitemaps"
    task :rebuild => [:environment, :build_from_config, :ping]
